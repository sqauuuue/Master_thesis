...
while year_start != year_end:
        ...
        while day != lastday_per_year:
            ...
            filename="Z%s" % (day)
            T=xr.load_dataset("%s/%s" % (datadir,filename)).to_array().loc[dict(variable="T",plev=plev,lon=lons,lat=lats)].squeeze()
            Q=xr.load_dataset("%s/%s" % (datadir,filename)).to_array().loc[dict(variable="Q",plev=plev,lon=lons,lat=lats)].squeeze()
            TD=np.log(Q*plev/100/(287.058/461.520+(1-287.058/461.520)*Q)/6.112)*243.5/(17.67-np.log(Q*plev/100/(287.058/461.520+(1-287.058/461.520)*Q)/6.112))+273.16
            data_in=np.exp(17.67*(TD-273.16)/(243.5+TD-273.16)-17.67*(T-273.16)/(243.5+T-273.16))*100
...
data_out.RH.attrs["_FillValue"]=False
