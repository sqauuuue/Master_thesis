...
VARNAME="T"        # available variables: "T": temperature, "Z": geopotential height, "U": zonal wind, "V": meridional wind, "Q": specific humidity
PLEV=$1         # available levels: 10000, 20000, 25000, 30000, 40000, 50000, 60000, 70000, 80000, 85000, 90000 Pa
...
ORISCRIPT=/home/kit/imk-tro/gj5173/era5_clim/era5_clim_JW.py                                       # the path+name to the python script doing the climatology calculations
...
SCRIPT=${OUTPUTDIR}/era5_clim_JW.py
cp ${ORISCRIPT} ${SCRIPT}
...
sed -i "83s,[^ ]*[^ ],data_out.${VARNAME}.attrs[\"_FillValue\"]=False,1" ${SCRIPT}
wait
